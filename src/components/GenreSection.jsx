...
const GenreSection = () => {
  ...
  const initialLimit = 4; // Number of products initially visible on mobile
  const initialLimitDesktop = 8; // Number of products initially visible on desktop

  // Detect screen width for slicing
  ...
  return (
    ...
    >
      ...
      {/* Tabs */}
      <div className="relative flex flex-wrap justify-center gap-3 sm:gap-8 mb-12 font-semibold text-black text-sm sm:text-base md:text-lg">
        {tabs.map((tab) => (
          <button
            key={tab}
            onClick={() => {
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-4 md:hidden">
              {(showAll
                ? productsByGenre[activeTab]
                : productsByGenre[activeTab].slice(0, sliceLimit)
              ).map((product, i) => (
                <motion.div
                  key={product._id}
